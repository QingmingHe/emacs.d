...
(setq project-roots
      `(("CExamples"
        ...
        ("SUGAR"
         ...
         :after-found-hdf5 (lambda ()
                             (project-root-set-data
                              :fortran-definitions
                              (append (project-root-data :fortran-definitions)
                                      '("USE_HDF5"))))
         :after-found-mpi (lambda ()
                            (project-root-set-data
                             :fortran-definitions
                             (append (project-root-data :fortran-definitions)
                                     '("USE_MPI")))))
        ("openmc"
         ...
         :include-tags ("~/.TAGS/mpi-f90" "~/.TAGS/hdf5-fortran")
         :use-completion (f90-mode)
         :use-packages (hdf5 mpi openmp))
        ("ctags"
         :root-contains-files (".ctags-root")
         :tags-tool "gtags"
         :c-include-paths ("." "main" "parsers")
         :c-definitions ("HAVE_CONFIG_H"))
        ("mcnp"
