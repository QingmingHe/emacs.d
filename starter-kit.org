...
(setq *win32* (string-match-p "win[0-9]+" (my-system-type)))
(setq *cygwin* (string-match-p "cygwin" (my-system-type)))
(setq *linux* (string-match-p "linux" (my-system-type)))
(setq *terminal* (eq window-system nil))

(if (< emacs-major-version 24)
    (error "Emacs major version should be >= 24.3")
  (when (< emacs-minor-version 3)
    (error "Emacs minor version should be >= 24.3"))
)
#+END_SRC
...
(add-hook 'emacs-startup-hook 'my-start-up-display)
#+END_SRC
