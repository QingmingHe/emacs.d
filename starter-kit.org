...
- Macros to lazily define hydra. As hydra.el is over one thousand lines, it
  consumes some time to load at Emacs start up. So define macros that only
  defines hydra until hydra body is called.
  ...
  (defmacro with-defhydra-evil-leader (name key &rest hydradef)
    (let ((name-defined (intern (format "defhydra-%s-defined-p" name)))
          (hydra-body-name (intern (concat name "/body"))))
      `(progn
         (setq ,name-defined nil)
...
** Starter kit core
   ...
   :END:
The following files contain the remainder of the core of the Emacs Starter
Kit. All of the code in this section should be loaded by everyone using the
starter kit. You should note that the sequence is very important.
#+BEGIN_SRC emacs-lisp
...
** Starter kit initial display and final settings

This should be placed at end of starter kit and configures the initial
windows.

*** Don't show Emacs tutorial
#+BEGIN_SRC emacs-lisp
