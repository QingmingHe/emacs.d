...
- Compile some packages that are installed manually

  #+begin_src emacs-lisp
  (mapc
   (lambda (pkg-command)
     (let ((path (file-name-directory (locate-library (car pkg-command)))))
       (when (file-exists-p path)
         (unless (was-compiled-p path)
           (let ((default-directory path))
             (call-process-shell-command (cdr pkg-command) nil nil nil))))))
   '(("evil" . "make")
     ("eim" . "make")))
  #+end_src
