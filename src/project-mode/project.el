...
(defvar prj/buffer-p nil
  "Buffer local `project-details'.")
(make-variable-buffer-local 'prj/buffer-p)
...
\"$PATH\" environment variable."
  ...
  (unless (executable-find "etags-update.pl")
    ...
    (shell-command
     (format "chmod 755 %s"
             (format "%s/etags-update.pl" prj/install-path)))))
...
(defun prj/update-tags-single-file ()
  ...
  (when (and
         (buffer-file-name))
    (if prj/use-gtags
        (prj/update-gtags-single-file prj/buffer-p)
      (etu/update-tags-for-file prj/buffer-p))))
...
(defun prj/helm-buffers-candidates ()
  (let (buffers
        (p (or prj/buffer-p (project-root-fetch))))
    (when p
      (mapc
       (lambda (buffer)
...
(defun prj/helm-create-new-file (file)
  (let* ((p (or prj/buffer-p (project-root-fetch)))
         (default-directory (cdr p)))
    (let ((default-directory
...
(define-minor-mode project-minor-mode
  ...
  (if project-minor-mode
      (progn
        (let ((p (or prj/buffer-p (setq prj/buffer-p (project-root-fetch))))
...
4. file is a project file."
  (let ((fname (buffer-file-name)))
    (when (and
           ...
           (file-exists-p fname)
           (not (file-remote-p fname))
           (setq prj/buffer-p (project-root-fetch)))
      (project-minor-mode))))
