...
(defun eshell-ido-complete-file (from b0 b1)
  "Use ido to complete read file name in eshell.
...
always fallback from file completion to directory by C-j."
  (let* ((file (ido-read-file-name ": " from))
    ...
    (if (string-match "\\`\\(\\.\\./\\)\\{3,\\}" relative)
        (insert file)
      (unless (string-match "\\`\\." relative)
        (insert "./"))
      (insert relative))))

(defun eshell-ido-or-helm ()
  "Use ido to complete file and helm or other in eshell.

Ido is activated when previous command is cp, cd, mv, rm, ... or thing at
point is a existing directory. Otherwise helm is used to complete."
  (interactive)
