...
(defun my-dired-present-heading ()
  (let ((buffer-read-only nil)
        file-num size available)
    (save-excursion
      (goto-char (point-min))
      (when (re-search-forward
             "total used in directory \\([^KMG]+[KMG]?\\) available \\([0-9]+\\)"
             nil t)
        (setq size (match-string-no-properties 1)
              available
              (/
               (/ (string-to-int (match-string-no-properties 2)) 1024.0)
               1024.0))
        (delete-region (line-beginning-position) (line-end-position))
        (insert (format "  files: %i used: %s available: %.1fG \n"
