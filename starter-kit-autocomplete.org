...
Company mode has backend for cmake-mode, but auto-complete doesn't. The
simplest way to add an ac-source for cmake-mode is to add a dictionary.
    ...
    (mapc
     (lambda (command)
       (mapc
        (lambda (keyword)
          (cond
           ((string-match "\\(.+GNU\\)<LANG>" keyword)
            (mapc
             (lambda (language)
               (insert (concat (match-string 1 keyword) language))
               (insert "\n"))
             '("CXX" "C")))
